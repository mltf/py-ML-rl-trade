...
def bt(num_features, use_existing_model, model_name):
    ...
    for t in range(num_features, l):
        ...
        reward, total_profits, total_holds, total_buys, total_sells = dqn.execute_action(action, data[t], t,
        ...
        if done:
            print("-----------------------------------------")
            print(
                f'Total Profit: {formatPrice(total_profits)} , Total hold/buy/exit trades: {total_holds} / {total_buys} / {total_sells}')
            print("-----------------------------------------")


stock_name = '^GSPC_2019'  # ^GSPC_2011  GSPC_2019 GSPC_1970_2019 GSPC_1970_2018
model_name = 'model_ep10000'  # model_ep0, model_ep10, model_ep20, model_ep30
